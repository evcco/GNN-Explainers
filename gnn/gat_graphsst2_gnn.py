...
from torch_geometric.nn import GATConv, BatchNorm, MessagePassing, global_mean_pool ,global_max_pool
...
class GraphSST2Net(torch.nn.Module):
    ...
    def get_node_reps(self, x, edge_index, edge_attr, batch):
        ...
        # x = F.dropout(x, p=0.6, training=self.training)
        node_x = self.conv2(x, edge_index)
        return node_x

    def get_graph_rep(self, x, edge_index, edge_attr, batch, edge_mask=None):
        node_x = self.get_node_reps(x, edge_index, edge_attr, batch)
        graph_x = global_mean_pool(node_x, batch)
        return graph_x
...
if __name__ == '__main__':
    ...
    save_path = 'GAT_graphsst2_net.pt'
