...
from cg.mask.edge_mask_net import EdgeMaskNet
...
class PGExplainer(Explainer):
    ...
    def __loss__(self, log_logits, mask, pred_label, criterion=torch.nn.CrossEntropyLoss()):
    ...
    def explain_graph(self, graph, model=None,
        ...
        if train_mode:
            ...
            return loss, G_acc.mean(dim=0).view(-1, 1), G1_acc, \
                   ...
                   float(G1_pos_edge_index.size(1)) / graph.edge_index.size(1)
